...
public:
	...
	QNetWorkThread(std::string UHost, unsigned int UPort, bool USSLEnadble, std::stringstream &URequestData, std::stringstream &UResponseData)
		:Host(UHost), Port(UPort), RequestData(URequestData), ResponseData(UResponseData), SSL_Enadble(USSLEnadble) {
		TimeOut = 5000;
		isEncryption = false;
	};
	/*
	...
	*/
	...
	void SetThreadControlSignal(unsigned int UCOntorl);
	void run();
public slots:
...
private:
	...
	unsigned int m_ControlSignal = 0;
	unsigned int WaitControlSignal();
